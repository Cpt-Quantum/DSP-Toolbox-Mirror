...
typedef enum {
	AMPLITUDE_SHAPE_FLAT,
	AMPLITUDE_SHAPE_TAIL
} wave_shape_t;

vector <wave_properties_t> wave_properties_gen(float sample_rate, unsigned int n_waves,
		float amplitude, wave_shape_t amplitude_shape, float dc_offset) {
	vector <wave_properties_t> waves;
	wave_properties_t wave_tmp;
	float freq_step = n_waves/sample_rate;
	...
	wave_tmp.phase = 0;
	wave_tmp.function_type = FUNCTION_SINE;
		...
		switch(amplitude_shape) {
			...
			case AMPLITUDE_SHAPE_TAIL:
				if (i == n_waves/2) {
					...
					wave_tmp.amplitude = amplitude * expf((i - (float)n_waves/2)/n_waves);
				}
				else if (i > n_waves/2) {
					wave_tmp.amplitude = amplitude * expf(((float)n_waves/2 - i)/n_waves);
				}
				break;
			default:
				break;
		}
		/* Set the rest of the wave properties before adding the struct to the vector */
...
int main (void) {
	...
	vector <wave_properties_t> waves_flat =
		wave_properties_gen(SAMPLE_RATE_HZ, SAMPLE_RATE_HZ/100, 100, AMPLITUDE_SHAPE_FLAT, 0);
	vector <wave_properties_t> waves_tail =
		wave_properties_gen(SAMPLE_RATE_HZ, SAMPLE_RATE_HZ/100, 100, AMPLITUDE_SHAPE_TAIL, 0);
