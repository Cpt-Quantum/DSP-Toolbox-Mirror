	...
	for (c = 0; c < length; c++) {
		...
		for (i = 0; i < N; i++) {
			data_out[c] = data_out[c] +
						  (filter->iir_coeff->b[i] *
						   filter->in_buff[(c + filter->cycle - i + N) % N]);
		}
		for (i = 1; i < N; i++) {
			data_out[c] = data_out[c] -
						  (filter->iir_coeff->a[i] *
...
void sos_filter(float *data_in, float *data_out, struct sos_filter_t *filter,
				uint32_t length) {
	...
	for (uint32_t i = 0; i < filter->num_sections; i++) {
		biquad_section_filter(data_in_buff, data_out,
