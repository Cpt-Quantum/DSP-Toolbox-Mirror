...
with open(f"{filename}.c", "w") as f:
    ...
    f.write('#include "fir_float.h"\n')
    f.write('#include "iir_float.h"\n\n')

    # ==========================================
    # Second Order Sections (SOS)
    # ==========================================
    f.write("/* Set the coefficients for each of the second order sections */\n")
    f.write("struct biquad_section_float_t biquads[] = {\n")
    ...
    f.write("struct sos_filter_float_t sos_filt = {\n")
    ...
    f.write(
        "struct iir_coeff_float_t iir_coeffs = {.a = iir_filt_a, .b = iir_filt_b};\n\n"
    )

    f.write("struct iir_float_t iir_filt = {.iir_coeff = &iir_coeffs,\n")
    ...
    f.write("struct fir_float_t fir_filt = {.b = fir_filt_coeff,\n")
    f.write("\t\t\t\t\t\t .in_buff = fir_filt_in_buff,\n")
    f.write("\t\t\t\t\t\t .taps = FIR_FILT_TAPS,\n")
    f.write("\t\t\t\t\t\t .cycle = 0};\n")
    f.write("float fir_filt_decimation_in_buff[FIR_FILT_TAPS] = {0};\n")
    f.write("struct fir_float_t fir_decimation_filt = {.b = fir_filt_coeff,\n")
...
with open(f"{filename}.h", "w") as f:
    ...
    f.write("extern struct sos_filter_float_t sos_filt;\n")
    f.write("extern struct iir_float_t iir_filt;\n")
    f.write("extern struct fir_float_t fir_filt;\n\n")
    f.write("extern struct fir_float_t fir_decimation_filt;\n\n")
